...
parser.add_option("-l", "--location", dest="location", 
    ...
    default="")
parser.add_option("-s", "--freq-shift-index", dest="freq_shift_idx", 
    help="frequency shift index of the DVB channel (default 1)",
    default=1)
...
idx = int(options.freq_shift_idx) #doesn't need to be changed (default 1)
#constants
...
#check input
if not(idx in range(0, len(freq_shift_kHz))):
  print("Frequency shift index is out of range (min 0, max "+str(len(freq_shift_kHz)-1)+")")
  exit(1)

#show summary
print("Scanning DVB-T channels from "+str(ch_min)+" to "+str(ch_max))
print("Carrier frequency offset "+str(freq_shift_kHz[idx])+" kHz")
